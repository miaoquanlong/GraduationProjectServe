...
module.exports = app => {
  ...
  app.route("/user")
     ...
     */
    .post((req, res) => {
      Users.findOne({ where: { username: req.query.username } }).then(result => {
        const salt = bcrypt.genSaltSync();
        req.query.password = bcrypt.hashSync(req.query.password, salt);
        if (!result) {
          res.json({ msg: '用户名不正确', isPrompt: true });
        } else if (req.query.password != result.dataValues.password) {
          res.json({ msg: '用户密码不正确', isPrompt: true });
          return
        } else {
          return res.json({
            success: true,
